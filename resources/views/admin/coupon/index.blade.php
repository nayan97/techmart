        ...
        <div class="container-fluid my-2">
            <div class="row mb-2">
                ...
                <div class="col-sm-6 text-right">
                    <a href="{{ route('discountcode.create')}}" class="btn btn-primary">New Coupons</a>
                </div>
            </div>
        </div>
        <!-- /.container-fluid -->
                    ...
                    <table class="table table-hover text-nowrap">
                        ...
                        <tbody>
                            @if ($discountCoupon->isNotEmpty())
                                @foreach ($discountCoupon as $item )
                                <tr>
                                    <td>{{$item->id}}</td>
                                    <td>{{$item->name}}</td>
                                    <td>{{$item->code}}</td>
                                    ...
                                    <td>
                                        <a href="{{ route('discountcode.edit', $item->id)}}">
                                            <svg class="filament-link-icon w-4 h-4 mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                                <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                                            </svg>
                                        </a>
                                        <a href="#" onclick="deleteCoupon({{$item->id}})" class="text-danger w-4 h-4 mr-1">
...
<script>
    function deleteCoupon(id){
        var url = '{{route("discountcode.destroy","ID")}}';
